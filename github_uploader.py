...
def run() -> None:
    ...
    # مفعّل افتراضياً لـ LinkedIn (مقاس مربع 1080×1080). يمكن تعطيله أو توسيعه
    # عبر EXTRA_PLATFORMS (قائمة مفصولة بفواصل من مفاتيح PLATFORM_SIZES في
    # image_generator.py، مثل: "linkedin,instagram_square").
    extra_platforms = [
        p.strip() for p in os.environ.get("EXTRA_PLATFORMS", "linkedin").split(",") if p.strip()
    ]
    for platform in extra_platforms:
        try:
            platform_paths = generate_platform_designs(content, out_dir, platform=platform)
